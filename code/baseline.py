...
def baseLine(mnist):
    ...
    for epoch in epoch_list:
        ...
        print("baseline训练轮数:", '%02d' % (epoch + 1), "Loss=", "{:.9f}".format(loss), "训练集准确率=", acc)

    # 使用测试集计算模型最终的准确率
    print("baseline测试集准确率:", sess.run(accuracy, feed_dict={x: mnist.test.images, y_label: mnist.test.labels}))
